...
class VDNModel(VideoBaseModel):

    def __init__(self, opt):
        super(VDNModel, self).__init__(opt)
        if self.is_train:
    ...
    def optimize_parameters(self, current_iter):
        ...
        super(VDNModel, self).optimize_parameters(current_iter)
    ...
    #             # logger.info(f'Params {name} will be optimized.')
    #         else:
    ...
    def test(self):
        ...
        self.net_g.eval()
        sf = self.opt['scale']
        self.tile = self.opt['val'].get('tile')
        self.tile_overlap = self.opt['val'].get('tile_overlap')
        self.window_size = self.opt['window_size']
        self.nonblind_denoising = self.opt['nonblind_denoising']
        ...
        with torch.no_grad():
            ...
            if num_frame_testing:
                ...
                num_frame_overlapping = self.tile_overlap[0]
                not_overlap_border = False
                b, d, c, h, w = self.lq.size()  # 100

                c = c - 1 if self.nonblind_denoising else c
                ...
                for d_idx in d_idx_list:
                    lq = self.lq[:, d_idx:d_idx + num_frame_testing, ...]
                    out_clip = self.test_patch(lq)
                ...
                d_old = self.lq.size(1) * sf
                d_pad = (window_size[0] - d_old % window_size[0]) % window_size[0]
                lq = torch.cat([self.lq, torch.flip(self.lq[:, -d_pad:, ...], [1])], 1) if d_pad else self.lq
                output = self.test_patch(lq)
                self.output = output[:, :d_old, :, :, :]
    ...
    def test_patch(self, lq):
        ...
        if size_patch_testing:
            ...
            overlap_size = self.tile_overlap[1]
            not_overlap_border = True
            # test patch by patch
            b, d, c, h, w = lq.size()
            c = c - 1 if self.opt['nonblind_denoising'] else c
