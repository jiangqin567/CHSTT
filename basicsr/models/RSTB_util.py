    ...
    def __init__(self, load_path=None, return_levels=[5]):
        ...
        if load_path:
            if not os.path.exists(load_path):
                ...
                url = '/home/jq/Trans/Trans-VSR/pretrained_models/spynet_sintel_final-3d2a1287.pth'
...
def window_partition(x, window_size):
    ...
    #            window_size[2], C)
    x = x.view(B,
               torch.div(D, window_size[0], rounding_mode='trunc'),
               window_size[0],
               torch.div(H, window_size[1], rounding_mode='trunc'),
               window_size[1],
               torch.div(W, window_size[2], rounding_mode='trunc'),
               window_size[2], C)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = self.qkv_self(x).reshape(B_, N, 3, self.num_heads, torch.div(C, self.num_heads, rounding_mode='trunc')).permute(2, 0, 3, 1, 4)
...
class Upsample(nn.Sequential):
    ...
    def __init__(self, scale, num_feat):   #4, 64
...
class DLA(nn.Module):
    def __init__(self, inp, oup, kernel_size=3, stride=1, expand_ratio=3, refine_mode='none'):
        ...
        self.bn1 = nn.BatchNorm3d(144)

        if refine_mode == 'conv':
            self.conv = nn.Conv3d(144, 144, 3, 1, 1)
...
class ResidualBlocksWithInputConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=144, num_blocks=30):
        super().__init__()
...
class FeedForward(nn.Module):
    def __init__(self, num_feat):
        super().__init__()
        self.conv2d_pre= torch.nn.Conv2d(in_channels=3, out_channels=num_feat, kernel_size=3, stride=1, padding=1)
        self.resblocks = ResidualBlocksWithInputConv(num_feat * 2, num_feat, num_blocks=30)
        self.fusion = torch.nn.Conv3d(in_channels=num_feat*2, out_channels=num_feat, kernel_size=3, padding=1, groups=1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=False)

    def forward(self, x, lrs, backward_flow,forward_flow):
        b, t, c, h, w = lrs.shape
        print(lrs.shape)
        embed_dims = x.shape[2]
        feats_backward = torch.zeros(b, t, embed_dims, h, w).cuda()
        # feat_prop = lrs.new_zeros(b, embed_dims, h, w).cuda()
        for i in range(t - 1, -1, -1):
            feat = x[:, i, :, :, :]
            # print(feat[0,0,0,0])
            if i < t - 1:
                flow = backward_flow[:, i, :, :, :]
                feat_prop = flow_warp(feat, flow.permute(0, 2, 3, 1))
                # print(feat_prop[0,0,0,0])
                pixel_prop = self.conv2d_pre(flow_warp(lrs[:, i + 1, :, :, :], flow.permute(0, 2, 3, 1)))
                # print(pixel_prop[0,0,0,0])
                feats_backward[:, i, :, :, :] = self.resblocks(torch.cat([feat_prop,pixel_prop],dim=1))
                # print(feats_backward[0,i,0,0,0])

            else:

                feats_backward[:, i, :, :, :] = feat
                # print(feats_backward[0, i, 0, 0, 0])

            # print(feats_backward[0, 0, 0, 0, 0])

        feats_forward = torch.zeros(b, t, embed_dims, h, w).cuda()
        ...
        for i in range(0, t):
            ...
            if i > 0:
                ...
                pixel_prop = self.conv2d_pre(flow_warp(lrs[:, i - 1, :, :, :], flow.permute(0, 2, 3, 1)))
                feats_forward[:, i, :, :, :] = self.resblocks(torch.cat([feat_prop, pixel_prop], dim=1))
            else:
                feats_forward[:, i, :, :, :] = feat_prop
                # print(feats_forward[0, i, 0, 0, 0])

        feat_trans = feats_backward + feats_forward
        return feat_trans


class globalAttention_v0(nn.Module):

    def __init__(self, num_feat, patch_size=8, heads=1):
        ...
        self.scale = 64 ** -0.5
        self.DLA = DLA(num_feat, num_feat, kernel_size=3, refine_mode='conv', expand_ratio=3)
        self.to_q3 = nn.Conv3d(in_channels=num_feat, out_channels=num_feat, kernel_size=3, padding=1,
                               groups=1)
        self.to_k3 = nn.Conv3d(in_channels=num_feat, out_channels=num_feat, kernel_size=3, padding=1,
                               groups=1)
...
#
#     def __init__(self, num_feat, feat_size,patch_size, heads):
#         super().__init__()
#         self.attn = Residual(PreNorm(num_feat, feat_size, globalAttention(num_feat, patch_size, heads)))
#
#     def forward(self, x):
#         x = self.attn(x)
#         return x
...
class Transformer(nn.Module):
    ...
    def __init__(self, num_feat, feat_size, patch_size, heads):
        super().__init__()
        self.attn1 = Residual(PreNorm(num_feat, feat_size, globalAttention(num_feat, patch_size, heads)))
        # self.attn2 = Residual(PreNorm(num_feat, feat_size, globalAttention(num_feat, patch_size, heads)))
    ...
    def __init__(self, num_feat, feat_size, depth, patch_size, heads):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                Residual(PreNorm(num_feat, feat_size, DANetHead(num_feat, feat_size))),
                Residual(PreNorm(num_feat, feat_size, FeedForward(num_feat))),
            ]))

    def forward(self, x, lrs=None, flows=None):
        for glattn, ff1, duattn, ff2 in self.layers:
            b, t, c, h, w = x.shape
            x = glattn(x)
            x = ff1(x, lrs=lrs, flows=flows)
            x = duattn(x.view(b * t, -1, h, w))
            x = x.view(b, t, -1, h, w)
            x = ff2(x, lrs=lrs, flows=flows)
...
#             # print(x.size())
#             # print(lrs.size())
#             # print(backward_flow.size())
#             # print(forward_flow.size())
#             x = ff(x, lrs, backward_flow, forward_flow)
#         return x
...
class DANetHead(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(mid_channels, mid_channels, 3, padding=1, bias=False),
                                    nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(mid_channels, mid_channels, 3, padding=1, bias=False),
                                    nn.ReLU())

        self.conv3 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(2*mid_channels, out_channels, 1))
...
class DAModule(nn.Module):
    ...
    def __init__(self,in_channels, out_channels):
        ...
        self.da_module = nn.Sequential(
            DANetHead(in_channels=in_channels, out_channels=out_channels),
            DANetHead(in_channels=in_channels, out_channels=out_channels),
            DANetHead(in_channels=in_channels, out_channels=out_channels),
            DANetHead(in_channels=in_channels, out_channels=out_channels),
            )
    ...
    def forward(self, tensor_input):
        return self.da_module(tensor_input)


# if __name__ == '__main__':
#     lrs = torch.randn(1,7,3,64,64).cuda()
#     feat = torch.randn(1,7,144,64,64).cuda()
#     backward_flow = torch.randn(1, 6, 2, 64, 64).cuda()
#     # print(backward_flow[0,0,0,0,0])
#     forward_flow = torch.randn(1, 6, 2, 64, 64).cuda()
#     # print(forward_flow[0,0,0,0,0])
#
#     net = Transformer(num_feat=144, feat_size= 144, patch_size=8, heads=1).cuda()
#     out = net(feat)
#
#
#     print(out[0, 0, 0, 0, 0])
