    ...
    def forward(self, lrs_):
        ...
        for i in range(t - 1, -1, -1):
            ...
            if i < t - 1:  # no warping required for the last timestep
                ...
                feat_prop = self.LTAM(lr_curr_feat, index_feat_buffer_s1, feat_prop, \
                                      sparse_feat_buffer_s1, sparse_feat_buffer_s2, sparse_feat_buffer_s3, \
                                      location_update)

                # add the location map
                if i in keyframe_idx_backward:
                    location_update = torch.cat([location_update, \
                                                 torch.stack([grid_x, grid_y], dim=0).type_as(lrs).expand(n, -1, -1,
                                                                                                          -1)],
                                                dim=1)  # n , 2t , h , w
        ...
        high_frequency_imgs = torch.stack(fina_out, dim=1)  # n,t,c,h,w
...
class Transformer(nn.Module):
    ...
    def __init__(self, patch_size=(8, 8), in_c=3, out_c=3):
        super(Transformer, self).__init__()
        self.patch_h = patch_size[0]
        self.patch_w = patch_size[1]
        assert self.patch_h == self.patch_w
        inchannels = in_c * self.patch_h * self.patch_w
        outchannels = out_c * self.patch_h * self.patch_w
        self.encoder = nn.Conv2d(in_c, inchannels, self.patch_h, self.patch_h, bias=False,
                                 groups=in_c)
        self.decoder = nn.ConvTranspose2d(outchannels, out_c, self.patch_h, self.patch_h,
                                          bias=False, groups=out_c)
        self.feat_extractor = RSTBWithInputConv(in_channels=inchannels,
                                                kernel_size=(1, 3, 3),
                                                groups=1,
                                                num_blocks=1,
                                                dim=inchannels,
        ...
        self.reconstruction = RSTBWithInputConv(in_channels=192 * 3,
                                                kernel_size=(1, 3, 3),
                                                groups=1,
                                                num_blocks=1,
                                                dim=192,
    ...
    def forward(self, bicubic_imgs, high_frequency_imgs, flows):
        b, t, c, h, w = bicubic_imgs.shape
        flows_forward, flows_backward = flows
        h_new = h // self.patch_h
        w_new = w // self.patch_w

        # resize flows
        if flows_forward is not None:
            flows_forward = resize_flow(flows_forward.view(-1, 2, h, w), size_type='shape',
                                        sizes=(h_new, w_new))  # torch.Size([4, 2, 32, 32])
            flows_forward = flows_forward.view(b, t - 1, 2, h_new, w_new)
        flows_backward = resize_flow(flows_backward.view(-1, 2, h, w), size_type='shape', sizes=(h_new, w_new))
        flows_backward = flows_backward.view(b, t - 1, 2, h_new, w_new)  # [n, t-1, 2, 32, 32]

        # encoding
        enco_bic_0 = self.encoder(bicubic_imgs.view(-1, c, h, w)).view(b, t, -1, h_new,
                                                                       w_new)  # torch.Size([5, 192, 32, 32])
        enco_hfi_0 = self.encoder(high_frequency_imgs.view(-1, c, h, w)).view(b, t, -1, h_new, w_new)

        ##feature extration
        feat_bic = self.feat_extractor(enco_bic_0)
        feat_hfi = self.feat_extractor(enco_hfi_0)  # torch.Size([1, 5, 192, 32, 32])

        b, t, C, h_new, w_new = feat_hfi.shape

        feat_backward_list = []
        feat_prop = enco_hfi_0.new_zeros(b, C, h_new, w_new)
        ...
        for i in range(t - 1, -1, -1):
            ...
            if i < t - 1:
                flow = flows_backward[:, i, :, :, :]
                feat_prop = flow_warp(feat_prop, flow.permute(0, 2, 3, 1), padding_mode='border')
                hfi_attn = self.attn(bic, hfi, hfi)
                feat_prop = self.attn(hfi_attn, feat_prop, feat_prop)

            feat_prop = torch.cat([hfi, feat_prop], dim=1)
            feat_prop = self.resblocks(feat_prop)
            feat_backward_list.append(feat_prop)  # (b,c,h,w)

        ##forward
        feat_backward_out = feat_backward_list[::-1]
        out_l = []
        feat_prop = torch.zeros_like(feat_prop)
        ...
        for i in range(t):
            ...
            out = torch.cat([feat_backward_out[i], hfi, feat_prop], dim=1)  # 192*3
            out_l.append(out)

        final_out = torch.stack(out_l, dim=1)
        final_out = self.reconstruction(final_out) + enco_hfi_0
        final_out = self.decoder(final_out.view(-1, C, h_new, w_new)).contiguous().view(b, t, c, h,
                                                                                        w) + high_frequency_imgs
        return final_out
...
class Attn_layer(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        qs = qs.view(N, 64, D, num).permute(0, 1, 3, 2).reshape(N, -1, D)
...
#     input = torch.randn(1, 15, 3, 256, 256).cuda()
#     net = FTVSR_denoising(scale=1).cuda()
