...
#################split the reds to 4 clips####################################
            if dataset_name.lower() in ['reds4','udm10','dvd']:
                max_index = len(val_data['lq'])
                num = 10
                split_num = max_index // num
                over_lap_frame = 1
                out_l = []
                data = {'lq': [], 'gt': []}
                ...
                for index in range(0, num):
                    if index == 0:
                        start_index = 0
                        end_index = split_num + over_lap_frame
                        data['lq'] = val_data['lq'][start_index:end_index, :, :, :]
                        data['gt'] = val_data['gt'][start_index:end_index, :, :, :]
                        data['lq'].unsqueeze_(0)
                        data['gt'].unsqueeze_(0)
                        self.feed_data(data)
                        self.test()
                        out_l.append(self.output[:,:-over_lap_frame,:,:,:])
                        # print(out_l[index].shape)

                    elif index > (num- 1):
                        start_index = split_num * index - over_lap_frame
                        end_index = split_num * (index + 1) + over_lap_frame
                        data['lq'] = val_data['lq'][start_index:end_index, :, :, :]
                        data['gt'] = val_data['gt'][start_index:end_index, :, :, :]
                        data['lq'].unsqueeze_(0)
                        data['gt'].unsqueeze_(0)
                        self.feed_data(data)
                        self.test()
                        out_l.append(self.output[:,over_lap_frame:-over_lap_frame,:,:,:])
                        # print(out_l[index].shape)

                    else:
                        start_index = split_num * index - over_lap_frame
                        end_index = split_num * (index + 1)
                        data['lq'] = val_data['lq'][start_index:end_index, :, :, :]
                        data['gt'] = val_data['gt'][start_index:end_index, :, :, :]
                        data['lq'].unsqueeze_(0)
                        data['gt'].unsqueeze_(0)
                ...
                self.output = torch.cat(out_l, dim=1)
                self.gt = val_data['gt'].unsqueeze_(0)
                visuals = self.get_current_visuals()
                self.gt = val_data['gt'].squeeze_(0)
                # print(self.gt.shape)
